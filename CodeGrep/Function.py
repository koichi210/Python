...
class MySearch:
    ...
    __root_path_name = default_path_name
    __file_name = default_file_name
    __include_list = []
    __exclude_list = []
    __exclude_replace_name = ["src", "dest"]

    def init(self):
        self.__include_list = []
        self.__exclude_list = []
        self.__exclude_replace_name = []

    def set_path_name(self, val):
        self.__root_path_name = val
    ...
    def set_exclude_codename_condition_replace_ownname(self, replace_src, replace_dest):
        self.__exclude_replace_name.append(replace_src)
        self.__exclude_replace_name.append(replace_dest)
    ...
    def __get_file_list(self):
        grep_file_name = self.__root_path_name + self.__file_name
    ...
    def __trim_exclude_replace_ownname(self, file_name, code):
        ...
        fname = fname.replace(
            self.__exclude_replace_name[0],
            self.__exclude_replace_name[1])

        code = [line for line in code if fname not in line]
        return code

    def __append_result(self, result_str, code, __file_name):
        result_str += __file_name + "\r\n"
